...
class Control {
  ...
  Future<Usuario> pegarUsuario() async {
    if (await this._db.pegarUsuario() != null) {
      setUsuario(await this._db.pegarUsuario());
    }
  }

  Future<Usuario> pegarUsuarioPorCredenciais(String email, String senha) async {
    Usuario a = await this._db.pegarUsuarioPorCredenciais(email, senha);
    print("A");
    print(a);
    await this._db.mantemUsuario(a);
    return a;
  }

  Future<int> tirarUsuario() async {
    Usuario a = this.usuario;
    setUsuario(null);
    return await this._db.limparUsuario();
  }
  ...
  Future<List<Produto>> pegarProdutosDoPedido(Pedido pedido) async {
    return await this._db.pegarProdutosDoPedido(pedido, this.usuario);
  }
